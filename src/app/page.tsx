    ...
    <div className="py-12 px-6 md:px-12 max-w-7xl mx-auto relative">
      ...
      {isLoading ? (
        ...
        <div className="relative">
          <div className="hidden lg:block absolute left-1/2 top-0 bottom-0 w-px bg-slate-200 dark:bg-red-950/30 shadow-[0_0_15px_rgba(0,0,0,0.05)] transition-colors"></div>

          <div className="columns-1 lg:columns-2 gap-16 lg:gap-24">
            {finalFilteredPosts.map((post) => {
              const plainTextSnippet = getCleanSnippet(post.content);

              return (
                <article key={post._id} className="break-inside-avoid mb-12 relative group bg-white dark:bg-black p-8 border border-[#E5E5E0] dark:border-red-950/30 shadow-sm hover:shadow-md dark:hover:shadow-[0_0_20px_rgba(153,27,27,0.1)] transition-all duration-500">
                  
                  {!isMidnight && <div className="absolute left-6 top-0 bottom-0 w-px bg-red-200"></div>}
                  
                  <div className={`relative z-10 ${!isMidnight ? 'pl-6' : ''}`}>
                    ...
                    <header className="mb-4 pr-8">
                      <Link href={`/entry/${post._id}`}>
                        <h2 className="text-3xl font-bold text-slate-900 dark:text-red-100 mb-2 leading-tight hover:underline decoration-slate-300 dark:decoration-red-900/50 break-words">{post.title}</h2>
                      </Link>
                      <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-zinc-600 italic">
                        <span>{post.isAnonymous ? 'Anonymous' : (typeof post.author === 'object' ? post.author.username : 'Author')}</span>
                        <span>•</span>
                    ...
                    <div className="prose prose-slate dark:prose-invert mb-2 leading-relaxed">
                      <p className="line-clamp-4 text-slate-600 dark:text-zinc-400 break-words">
                        {plainTextSnippet}
                      </p>
                    </div>
                    ...
                    {post.tags && post.tags.length > 0 && (
                      <div className="flex gap-2 flex-wrap mb-6">
                        {post.tags.map((tag, index) => (
                          <span key={index} className="text-xs text-slate-500 dark:text-zinc-600 border border-slate-200 dark:border-red-950/30 px-2 py-1 rounded-sm">
                            #{tag}
                          </span>
                        ))}
                      </div>
